...
def train_lequa(
    ...
    if dataset == "T2":
        ...
        common_param_path = os.path.join(parameters_path, "common_parameters_EEG.json")
        sample_size = 20
        n_features = 5
        n_classes = 8
        path = os.path.join(data_path, "dataset/ChbMit")

        dataset_full = EEGDataset(folder_path=path, window_size=sample_size)
        train_size = int(0.8 * len(dataset_full))
        val_size = len(dataset_full) - train_size
        dataset_train, dataset_val = torch.utils.data.random_split(dataset_full, [train_size, val_size])

        train_prevalences = torch.stack([y for _, y in dataset_train])
        val_prevalences = torch.stack([y for _, y in dataset_val])
        loss = MRAE(eps=1.0 / (2 * sample_size), n_classes=n_classes)
    ...
    elif feature_extraction == "timeseriescnn":
        fe = TimeSeriesCNN(input_channels=n_features, output_size=network_parameters.pop("fe_output_size"))
    ...
    if dataset == "EEG":
        train_bag_generator = None  # Will not be used, GMNet takes the raw dataset
        val_bag_generator = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", help="Dataset to use: T1B, T2, T3, EEG", required=True)
