    ...
    def fit(self, dataset: Dataset, val_dataset: Dataset = None):
        ...
        if not self.checkpoint_loaded:
            ...
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                ...
                eta_min=self.end_lr        # final learning rate
            )
        ...
        for epoch in range(self.initial_epoch, self.train_epochs):
            ...
            for batch_i, batch in enumerate(train_dataloader):
                ...
                with torch.cuda.amp.autocast(enabled=self.use_fp16):
                    ...
                    )  # shape: [B, K, C*K] expected

                    # === Handle output shape [B, K, C*K] ===
                    if p_hat.dim() == 3:  # [K, B, C*K]
                        K, B, CK = p_hat.shape
                        C = self.n_classes
                        assert CK % C == 0
                        K_per_class = CK // C
                        ...
                        # Reshape to [B, K, C, K_per_class]
                        p_hat = p_hat.view(B, K, C, K_per_class)

                        # Mean over K samples and Gaussians per class → [B, C]
                        p_hat_avg = p_hat.mean(dim=1).mean(dim=2)

                        # Optional safety: normalize
                        p_hat_avg = p_hat_avg / (p_hat_avg.sum(dim=1, keepdim=True) + 1e-8)
                    elif p_hat.dim() == 2:
                        # Already [B, C], no processing needed
                        p_hat_avg = p_hat
                    else:
                    ...
                    if "y" in batch:
                        yb = self.__move_data_device(batch["y"]).float()
                        pos = yb.mean(dim=1, keepdim=True)
                        neg = 1.0 - pos
                        p_true = torch.cat([neg, pos], dim=1)  # binary prevalence
                    else:
                    ...
                    #print(f"    DEBUG shapes p_true:{p_true.shape}, p_hat_avg:{p_hat_avg.shape}")
            ...
            # validation
            if self.is_val_set:
                val_quant = self.__compute_validation_loss(val_dataloader, epoch)
            ...
            if self.save_model_path:
                ...
                print(f"[{self.dataset_name}] Last model checkpoint saved to {self.save_model_path}")
        ...
        if self.save_model_path and os.path.isfile(self.save_model_path):
            print(f"[{self.dataset_name}] Loading last model checkpoint from {self.save_model_path}")
            self.model.load_state_dict(torch.load(self.save_model_path))
    ...
    def predict(self, dataset, process_in_batches=0):
        with torch.no_grad():
            if process_in_batches == 0:
                ...
                for i, batch in enumerate(test_dataloader):
                    print(f"Processing batch {i} of {len(test_dataloader)}")
                    X = self.__move_data_device(batch[0])
                    raw = self.model(input=X, return_classification=False)  # [M, B, C*K]
                    if raw.dim() == 3:
                        raw = raw.permute(1, 0, 2)  # [B, M, C*K]
                        B, M, CK = raw.shape
                        C = self.n_classes
                    ...
                    predictions[i * process_in_batches: i * process_in_batches + B] = raw
